...
TOTAL_LAYERS = 4    # total number of layers used in the network 
               # TOTAL_LAYERS = ( 1 conv + 2 * residual blocks + 1 avg pool )
...
def inference(images, is_training):
    ...
    print ('RESIDUAL NET TOTAL LAYERS: %d' % TOTAL_LAYERS)
    ...
    size_after_conv = int(h_conv.get_shape()[1])
    # Avgpool
    h_pool = tf.nn.avg_pool(h_conv, ksize=[1, size_after_conv, size_after_conv, 1], 
                            strides=[1, size_after_conv, size_after_conv, 1], 
                            padding='VALID', name='h_pool')
    features_total = int(h_pool.get_shape()[-1])
    h_pool = tf.reshape(h_pool, [-1, features_total])
    ...
    with tf.variable_scope('fc'):
        ...
        biases = tf.Variable(tf.zeros([NUM_CLASSES]),
                             name='biases')
        logits = tf.matmul(h_pool, weights) + biases
